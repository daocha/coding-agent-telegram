#!/usr/bin/env bash
# One-time environment setup: creates/activates the venv, installs the
# package, creates the .env file if missing, and validates required
# settings. Does not start the bot itself -- ./startup.sh runs this once
# and then supervises the actual bot process (restarting it across crashes
# and network outages). Run this directly only if you want to (re)prepare
# the environment without starting anything.
