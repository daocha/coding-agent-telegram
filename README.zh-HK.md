...
即使 `claude auth status` 顯示你已登入，甚至你啱啱重新登入之後，呢個問題都可能出現。Claude Code 平時用嘅互動式 OAuth session，可能會單獨喺呢個 bot 建立 session 所用嘅 headless、獨立子程序方式下失效，而同你實際嘅登入狀態無關——呢種情況喺 Claude Code CLI 自動更新之後出現過，亦有可能係間歇性嘅。

如果用戶透過 `/new`、恢復某個 session，或者發送任何訊息即時遇到咗呢個問題，bot 會識別出呢個特定嘅失敗，並即刻回覆修復說明，而唔係原始嘅 CLI 錯誤。

**修復方法**：喺運行呢個 bot 嘅主機上：

1. 執行 `claude setup-token`，並喺開咗嘅瀏覽器度批准存取。**真正嘅 token 之後會印喺你嘅終端機（以 `sk-ant-oat01-` 開頭），瀏覽器唔會顯示**——喺瀏覽器頁面本身而唔係終端機度複製嘢係常見嘅錯誤，咁樣係唔work嘅。呢個會建立一個長期有效（約 1 年）嘅驗證權杖——呢個係 Anthropic 官方支援嘅 headless/自動化用途機制（同 GitHub Actions 用嘅一樣）。同互動式 OAuth session 唔同，佢唔依賴於喺獨立背景程序中正常刷新 Keychain/session，所以設定一次之後，預計喺到期之前都唔使再處理。
2. 複製列印出嚟嘅權杖，然後根據你運行呢個應用程式嘅方式，用對應嘅指令儲存佢——bot 自身嘅回覆會自動選擇正確嘅指令，呢度列出嚟供參考：
   - 透過 `pip` 或一行式 `install.sh` 安裝（Quick Start 方案 A/B）：`coding-agent-telegram claude-auth <token>`
   - 從 clone 落嚟嘅 repository 用 `./startup.sh` 運行（Quick Start 方案 C）：`./startup.sh claude-auth <token>`

   呢兩個指令都會將權杖以 `CLAUDE_CODE_OAUTH_TOKEN` 嘅形式儲存到你嘅 env 檔案入面，並即刻重新驗證 Claude 認證，所以你會即刻知道成功定失敗，而唔使盲目重啟然後乾等。
3. 重啟 bot，等緊喺運行緊嘅程序套用呢個改動。

你亦都可以跳過呢個指令，直接喺你嘅 env 檔案（`.env_coding_agent_telegram`）入面自行設定 `CLAUDE_CODE_OAUTH_TOKEN=<token>`——以上兩個指令只係對呢個操作嘅方便封裝，加埋自動驗證。
</details>
